# Script to build base image for ace3p
#  - installs python3
#  - clones source files
# docker build -f python3.dockerfile -t johnkit/cmb-ace3p-win-base -m 20g --no-cache .
...
RUN git clone --recursive --branch ace3p-production https://gitlab.kitware.com/john.tourtellott/cmb-superbuild.git
RUN cd cmb-superbuild; git log -1
RUN cd C:/Users/ContainerUser/code/cmb-superbuild/superbuild; git remote add john https://gitlab.kitware.com/john.tourtellott/common-superbuild.git; git fetch john; git reset --hard john/ace3p-production; git log -1
